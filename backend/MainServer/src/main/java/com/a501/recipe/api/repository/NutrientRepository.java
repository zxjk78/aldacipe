...
public interface NutrientRepository extends JpaRepository<Nutrient,String> {
    ...
    @Query("select rn.nutrient from RecommendedIntake rn where rn.gender=:gender and rn.minAge <= :age and rn.maxAge >= :age and rn.type=:recommendedType")
    Optional<Nutrient> searchRecommendedIntakeNutrientByUser(@Param("gender") Gender gender, @Param("age") int age,@Param("recommendedType") RecommendedNutrientType type);
}
